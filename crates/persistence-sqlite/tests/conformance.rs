//! Conformance test suite for SQLite persistence adapter.
//!
//! Tests that all M1 aggregates (Question and Answer) can be persisted to SQLite
//! and retrieved with full fidelity, including revisions, votes, and credentials.

use persistence_sqlite::SqlitePersistence;
...
fn test_question_update_preserves_identity() {
    ...
    match retrieved {
        PersistableAggregate::Question(retrieved_q) => {
            assert_eq!(retrieved_q.id(), QuestionId::new(1));
            assert_eq!(retrieved_q.current_body(), q.current_body());
